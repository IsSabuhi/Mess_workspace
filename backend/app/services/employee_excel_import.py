"""Парсинг Excel со списком сотрудников (колонки учётной записи, ФИО, должности)."""
...
_POSITION_HEADERS = frozenset({"должность"})
    ...
    try:
        ...
        for r in range(header_row_idx + 1, (ws.max_row or 0) + 1):
            ...
            if not login and not full_name:
                continue
            out.append(ParsedEmployeeRow(sheet_row=r, login=login, full_name=full_name, position_title=pos_t))

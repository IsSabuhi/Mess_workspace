...
export function Sidebar({ collapsed, onToggleCollapse }: Props) {
  ...
  const [taskBoardsOpen, setTaskBoardsOpen] = useState(
    () => localStorage.getItem(TASK_BOARDS_OPEN_KEY) !== '0',
  );
  ...
  const showAdmin =
    state.status === 'authenticated' && canAdminAccess(state.user);
  const showSchedule =
    state.status === 'authenticated' && canViewSchedule(state.user);
              ...
              state.status === 'authenticated' && (
                <div className='-mt-1 mb-1 ml-11 space-y-1'>
                  <button
                    ...
                    <span>Доски</span>
                    <ChevronDown
                      className={`h-3.5 w-3.5 transition ${taskBoardsOpen ? '' : '-rotate-90'}`}
                    />
                  </button>
                  {taskBoardsOpen && (
                    <>

...
export function ManagerTeamDashboardPage() {
  ...
  const byColumn = useMemo(() => groupTasksByColumn(filtered), [filtered]);
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            <label className="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-300">
              ...
              <input
                ...
                className="rounded border border-slate-200 bg-white px-1.5 py-1 dark:border-slate-600 dark:bg-slate-800"
              />
            </label>
            <label className="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-300">
              До
            ...
            <input
              ...
              className="min-w-[15rem] rounded border border-slate-200 bg-white px-2 py-1 text-xs dark:border-slate-600 dark:bg-slate-800"
            />
            <button
              type="button"
              onClick={() => setFilters(DEFAULT_TASK_ANALYTICS_FILTERS)}
              className="rounded bg-slate-100 px-2.5 py-1 text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-200"
            >
              Сбросить
            </button>
            <button
              ...
              className="rounded bg-sky-100 px-2.5 py-1 text-xs text-sky-700 dark:bg-sky-950/40 dark:text-sky-300"
            >
              Экспорт CSV
            </button>
          </div>
            ...
            <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-5 dark:border-slate-700 dark:bg-slate-900/60">
              ...
              <div className="mt-4">
                <Suspense fallback={<ChartSkeleton className="h-52" />}>
                  <ManagerSystemsChart rows={bySystem.map((r) => ({ name: r.name, total: r.total, overdue: r.overdue }))} dark={chartDark} />
                </Suspense>
              </div>
            </div>
            <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-5 dark:border-slate-700 dark:bg-slate-900/60">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="font-semibold">По колонкам</h3>
              </div>
              <div className="space-y-1 text-sm">
                {byColumn.map((r) => (
                  <div key={r.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-2 py-1 dark:bg-slate-800/60">
                    <span>{r.name}</span>
                    <span className="text-xs text-slate-500">всего {r.total} · активн. {r.active}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {!allTasksQuery.isPending && tab === "workload" && (
          <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-5 dark:border-slate-700 dark:bg-slate-900/60">
            ...
            <p className="mb-3 text-xs text-slate-500">Учитываются текущие фильтры; красная часть показывает просроченные задачи.</p>
